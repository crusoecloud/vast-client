# Cluster

## Properties
Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**Id** | **int32** |  | [optional] [default to null]
**AvailableUpgradeVersion** | [***ClusterAvailableUpgradeVersion**](Cluster_available_upgrade_version.md) |  | [optional] [default to null]
**TurboBoostFlag** | **bool** | if enabled VMS will enable turbo boost during upgrade | [optional] [default to null]
**S3NewVersion** | [***interface{}**](interface{}.md) |  | [optional] [default to null]
**Guid** | **string** |  | [optional] [default to null]
**Ip** | **string** | Cluster Leader IP address | [optional] [default to null]
**MgmtVip** | **string** | Management VIP | [optional] [default to null]
**MgmtCnode** | **string** | Management CNode | [optional] [default to null]
**MgmtInnerVip** | **string** | VMS inner (NFS) vip | [optional] [default to null]
**MgmtInnerVipCnode** | **string** | VMS inner VIP CNode | [optional] [default to null]
**Name** | **string** |  | [default to null]
**Psnt** | **string** |  | [optional] [default to null]
**State** | **string** | Cluster state: INIT/ACTIVATING/ONLINE/OFFLINE/FAILED/UNKNOWN (displayed in GUI as disconnected) | [optional] [default to null]
**LeaderState** | **string** | Cluster Leader state: UP/DOWN/UNKNOWN | [optional] [default to null]
**SsdRaidRebuildProgress** | **int32** |  | [optional] [default to null]
**NvramRaidRebuildProgress** | **int32** |  | [optional] [default to null]
**NvramRaidRebuildProgressFraction** | **string** |  | [optional] [default to null]
**MemoryRaidRebuildProgress** | **int32** |  | [optional] [default to null]
**RaidRebuildProgress** | **string** |  | [optional] [default to null]
**QuotasAllocatedCapacity** | **int32** |  | [optional] [default to null]
**QuotasUsedCapacity** | **int32** |  | [optional] [default to null]
**QuotasUsedPercent** | **float32** |  | [optional] [default to null]
**RaidDrivesCanFail** | **int32** |  | [optional] [default to null]
**SsdRaidState** | **string** |  | [optional] [default to null]
**NvramRaidState** | **string** |  | [optional] [default to null]
**NvramSize** | **int32** |  | [optional] [default to null]
**MemoryRaidState** | **string** |  | [optional] [default to null]
**RemainingStripesHealth** | **string** |  | [optional] [default to null]
**ReplicationBwMb** | **float32** |  | [optional] [default to null]
**ReplicationIops** | **float32** |  | [optional] [default to null]
**ReplicationLatencyMs** | **float32** |  | [optional] [default to null]
**ReplicationRdBwMb** | **float32** |  | [optional] [default to null]
**ReplicationRdIops** | **int32** |  | [optional] [default to null]
**ReplicationRdLatencyMs** | **float32** |  | [optional] [default to null]
**ReplicationWrBwMb** | **float32** |  | [optional] [default to null]
**ReplicationWrIops** | **int32** |  | [optional] [default to null]
**ReplicationWrLatencyMs** | **float32** |  | [optional] [default to null]
**MdUsageHealth** | **string** |  | [optional] [default to null]
**Build** | **string** | Cluster build | [optional] [default to null]
**SystemName** | **string** | Internal guid string used by cluster s/w | [optional] [default to null]
**PhysicalSpace** | **int64** | Total Physical Space | [optional] [default to null]
**PhysicalSpaceInUse** | **int64** | Physical Space In Use | [optional] [default to null]
**PhysicalSpaceWoOverhead** | **int64** | Usable Space | [optional] [default to null]
**UsableCapacityBytes** | **int64** | Usable Capacity | [optional] [default to null]
**FreeUsableCapacity** | **int64** | Free Usable Capacity | [optional] [default to null]
**UsableAuxiliarySpaceInUse** | **float32** | Usable Auxiliary Space In Use | [optional] [default to null]
**UsableCapacityTb** | **float32** | Usable Capacity TB | [optional] [default to null]
**FreeUsableCapacityTb** | **float32** | Free Usable Capacity TB | [optional] [default to null]
**PhysicalSpaceInUseWoOverhead** | **int64** | Usable Space In Use | [optional] [default to null]
**LogicalSpace** | **int64** | Total Estimated Logical Space | [optional] [default to null]
**LogicalSpaceInUse** | **int64** | Logical Space In Use | [optional] [default to null]
**FreeLogicalSpace** | **float32** | Free Logical Space | [optional] [default to null]
**FreeLogicalSpaceTb** | **float32** | Free Logical Space | [optional] [default to null]
**FreePhysicalSpace** | **float32** | Free Physical Space | [optional] [default to null]
**FreePhysicalSpaceTb** | **float32** | Free Physical Space | [optional] [default to null]
**PhysicalSpaceTb** | **float32** | Total Physical Space | [optional] [default to null]
**FreePhysicalSpaceWoOverhead** | **float32** | Free Usable Space | [optional] [default to null]
**FreePhysicalSpaceWoOverheadTb** | **float32** | Free Usable Space In Use | [optional] [default to null]
**AuxiliarySpaceInUse** | **float32** | Physical Snapshot Space In Use | [optional] [default to null]
**AuxiliarySpaceInUseTb** | **float32** | Physical Snapshot Space In Use (TB) | [optional] [default to null]
**LogicalAuxiliarySpaceInUse** | **float32** | Logical Snapshot Space In Use | [optional] [default to null]
**LogicalAuxiliarySpaceInUseTb** | **float32** | Logical Snapshot Space In Use (TB) | [optional] [default to null]
**EstoreCapacityInUseBytes** | **float32** | Logical Space In Use | [optional] [default to null]
**EstoreCapacityInUseTb** | **float32** | Logical Space In Use (TB) | [optional] [default to null]
**PhysicalSpaceInUseTb** | **float32** | Physical Space In Use | [optional] [default to null]
**LogicalSpaceTb** | **float32** | Total Estimated Logical Space | [optional] [default to null]
**LogicalSpaceInUseTb** | **float32** | Logical Space In Use | [optional] [default to null]
**PhysicalSpaceInUsePercent** | **float32** | Physical space in use percentage | [optional] [default to null]
**LogicalSpaceInUsePercent** | **float32** | Logical space in use percentage | [optional] [default to null]
**LogicalDrrPercent** | **float32** | Logical data reduction ratio percentage | [optional] [default to null]
**LogicalInodesInUseNum** | **int32** |  | [optional] [default to null]
**PhysicalDrrPercent** | **float32** | Physical data reduction ratio percentage | [optional] [default to null]
**Drr** | **float32** | Data reduction ratio (logical_space_in_use / physical_space_in_use) | [optional] [default to null]
**DrrText** | **string** | DRR presented as 1:5.6 | [optional] [default to null]
**RdIops** | **int64** | Read IOPS | [optional] [default to null]
**WrIops** | **int64** | Write IOPS | [optional] [default to null]
**NdbBandwidth** | **float32** | Vast DB query bandwidth total (MB) | [optional] [default to null]
**NdbBandwidthRead** | **float32** | Vast DB query bandwidth read (MB) | [optional] [default to null]
**NdbBandwidthWrite** | **float32** | Vast DB query bandwidth write (MB) | [optional] [default to null]
**NdbNumberOfRunningQueries** | **int64** | Vast DB number of running queries | [optional] [default to null]
**NdbRowsScannedPerSecond** | **int64** | Vast DB rows scanned per second | [optional] [default to null]
**Nfs4Certificate** | **string** |  | [optional] [default to null]
**Nfs4PrivateKey** | **string** |  | [optional] [default to null]
**Iops** | **int64** | IOPS | [optional] [default to null]
**RdMdIops** | **int64** | Read Meta-data IOPS | [optional] [default to null]
**WrMdIops** | **int64** | Write Meta-data IOPS | [optional] [default to null]
**MdIops** | **int64** | Meta-data IOPS | [optional] [default to null]
**RdBw** | **int64** | Read Bandwidth | [optional] [default to null]
**WrBw** | **int64** | Write Bandwidth | [optional] [default to null]
**Bw** | **int64** | Bandwidth | [optional] [default to null]
**RdBwMb** | **float32** | Read Bandwidth | [optional] [default to null]
**WrBwMb** | **int32** | Write Bandwidth | [optional] [default to null]
**BwMb** | **int32** | Bandwidth | [optional] [default to null]
**RdLatency** | **int64** | Read Latency | [optional] [default to null]
**WrLatency** | **int64** | Write Latency | [optional] [default to null]
**Latency** | **int64** | Latency | [optional] [default to null]
**RdLatencyMs** | **int32** | Read Latency | [optional] [default to null]
**WrLatencyMs** | **int32** | Write Latency | [optional] [default to null]
**LatencyMs** | **int32** | Latency | [optional] [default to null]
**Shards** | **int32** | Number of shards | [optional] [default to null]
**DrShards** | **int32** | Number of DR shards | [optional] [default to null]
**DrWbShards** | **int32** | Number of DR write buffer shards | [optional] [default to null]
**StripeGroups** | **int32** | Number of stripe groups | [optional] [default to null]
**MicroShards** | **int32** | Number of shards | [optional] [default to null]
**MicroDrShards** | **int32** | Number of DR shards | [optional] [default to null]
**MicroDrWbShards** | **int32** | Number of DR write buffer shards | [optional] [default to null]
**MicroStripeGroups** | **int32** | Number of stripe groups | [optional] [default to null]
**MegaShards** | **int32** | Number of shards | [optional] [default to null]
**MegaDrShards** | **int32** | Number of DR shards | [optional] [default to null]
**MegaDrWbShards** | **int32** | Number of DR write buffer shards | [optional] [default to null]
**MegaStripeGroups** | **int32** | Number of stripe groups | [optional] [default to null]
**CnodeCores** | **int32** | Number of cnode cores | [optional] [default to null]
**ClusterPrivateKey** | **string** |  | [optional] [default to null]
**ClusterCertificate** | **string** |  | [optional] [default to null]
**CnodeMetrics** | **bool** |  | [optional] [default to null]
**MaxNvramReplicationFactor** | **int32** | Max NVRAM replication factor | [optional] [default to null]
**MaxPerformance** | [***ClusterMaxPerformance**](Cluster_max_performance.md) |  | [optional] [default to null]
**MaxSsdCapacityPercent** | **int32** | Max SSD capacity percent | [optional] [default to null]
**MaxNvramCapacityPercent** | **int32** | Max NVRM capacity percent | [optional] [default to null]
**DboxHaSupport** | **bool** | DBox HA support | [optional] [default to null]
**DisableDirsnap** | **bool** | Disable directory snapshots | [optional] [default to null]
**DisableMetrics** | **bool** |  | [optional] [default to null]
**DeleteSnapBlocksThreshold** | **int32** |  | [optional] [default to null]
**Loopback** | **bool** | Loopback (single node) installation | [optional] [default to null]
**Enabled** | **bool** | cluster enabled | [optional] [default to null]
**PerfCheck** | **bool** | Check cluster performance | [optional] [default to null]
**Dmsetup** | **bool** | Mock NVMeoF devices with dmsetup devices | [optional] [default to null]
**EnableDr** | **bool** | enable data reduction | [optional] [default to null]
**EnableEncryption** | **bool** | enable data encryption | [optional] [default to null]
**AllowEncryption** | **bool** | allow data encryption | [optional] [default to null]
**HalfSystem** | **bool** | Is half system | [optional] [default to null]
**UsedHandlesPercent** | **float32** |  | [optional] [default to null]
**UsedHandlesCount** | **int32** |  | [optional] [default to null]
**UseSpdk** | **bool** | Should install spdk | [optional] [default to null]
**DeepStripe** | **bool** | Is deep stripe system | [optional] [default to null]
**DrHashSize** | **int32** | DR hash size in buckets | [optional] [default to null]
**DrivePciPortType** | **string** | Cluster pci port type (dual, single) | [optional] [default to null]
**DriveSize** | **int32** |  | [optional] [default to null]
**SshUser** | **string** | SSH User name | [optional] [default to null]
**LeaderCnode** | **string** | Leader CNode | [optional] [default to null]
**DefragThreshold** | **int32** | Defrag Threshold | [optional] [default to null]
**EnableSmb** | **bool** | enable SMB | [optional] [default to null]
**EnableS3** | **bool** |  | [optional] [default to null]
**EnableSimilarity** | **bool** | enable similarity | [optional] [default to null]
**FormatDrives** | **bool** | Format drives | [optional] [default to null]
**ExpansionState** | **string** |  | [optional] [default to null]
**ExpansionPhase** | **int32** |  | [optional] [default to null]
**ExpansionPhaseDescription** | **string** |  | [optional] [default to null]
**Url** | **string** |  | [optional] [default to null]
**Uptime** | **string** | Cluster uptime | [optional] [default to null]
**OnlineStartTime** | **string** | Cluster online start time | [optional] [default to null]
**DeploymentTime** | **string** | Cluster deployment time | [optional] [default to null]
**SystemSettings** | [***interface{}**](interface{}.md) |  | [optional] [default to null]
**UpgradePhase** | **string** | Phase of a background running upgrade | [optional] [default to null]
**UpgradeProgress** | [***ClusterUpgradeProgress**](Cluster_upgrade_progress.md) |  | [optional] [default to null]
**UpgradeState** | **string** |  | [optional] [default to null]
**EnableTrash** | **bool** |  | [optional] [default to null]
**Title** | **string** |  | [optional] [default to null]
**TrashGid** | **int32** | GID with permissions to the trash folder (deprecated since 4.6) | [optional] [default to null]
**MockFanoutAuth** | **bool** | (deprecated since 4.6) | [optional] [default to null]
**S3AllowCors** | **bool** |  | [optional] [default to null]
**S3Certificate** | **string** |  | [optional] [default to null]
**S3PrivateKey** | **string** |  | [optional] [default to null]
**AllowNfs3OverUdp** | **bool** | Allow/deny NFS3 over UDP | [optional] [default to null]
**EnableTurboBoost** | **bool** | Enable Turbo Boost | [optional] [default to null]
**ShowmountSuppressed** | **bool** | Suppress Linux showmount command output on for NFSv3 users | [optional] [default to null]
**SwDefined** | **bool** |  | [optional] [default to null]
**SwVersion** | **string** |  | [optional] [default to null]
**LeaderUpgradeState** | **string** |  | [optional] [default to null]
**ProvidesBlocked** | **bool** | Block/Unblock all external providers | [optional] [default to null]
**AutoLogoutTimeout** | **int32** | Inactivity period for auto logout for VAST OS on all nodes (seconds) | [optional] [default to null]
**Motd** | **string** | Custom Message of the Day text displayed by VAST OS on login | [optional] [default to null]
**MotdAppendToDefault** | **bool** | Append custom motd to default motd | [optional] [default to null]
**UseSmbPrivilegedUser** | **bool** | If true, the privileged user is enabled. (deprecated since 4.6) | [optional] [default to null]
**SmbPrivilegedUserName** | **string** | Optional domain user logon name. If not set, the privileged user is &#x27;vastadmin&#x27;. (deprecated since 4.6) | [optional] [default to null]
**UseSmbPrivilegedGroup** | **bool** | If true, the privileged group is enabled. (deprecated since 4.6) | [optional] [default to null]
**SmbPrivilegedGroupFullAccess** | **bool** | If true, the privileged group has full access. Otherwise, read only. (deprecated since 4.6) | [optional] [default to null]
**SmbPrivilegedGroupSid** | **string** | Optional custom SID to specify a non default privileged group. If not set, privileged group is the Backup Operators domain group. (deprecated since 4.6) | [optional] [default to null]
**SmbAdministratorsGroupName** | **string** | Optional custom group logon name. If set, the group name will replace local administrators group access. (deprecated since 4.6) | [optional] [default to null]
**RewriteStatus** | [***interface{}**](interface{}.md) | Rewrite status summary | [optional] [default to null]
**RewritePhase** | **string** | Current Rewrite phase | [optional] [default to null]
**RewriteProgress** | **int32** | Current Rewrite phase progress (%) | [optional] [default to null]
**RewriteType** | **[]string** | Current Rewrite types | [optional] [default to null]
**RootCertificate** | **string** |  | [optional] [default to null]
**AuditDirName** | **string** | Name of the audit directory, which is located under the root directory. | [optional] [default to null]
**ReadAccessUsers** | **[]string** | Enter users here to grant them read access to all files in the audit directory. To make the audit directory accessible to clients, create a view on the directory. | [optional] [default to null]
**ReadAccessUsersGroups** | **[]string** | Enter groups here to grant them read access to all files in the audit directory. To make the audit directory accessible to clients, create a view on the directory. | [optional] [default to null]
**MaxFileSize** | **int32** | Maximum audit file size for each CNode core | [optional] [default to null]
**MaxHandlesCount** | **int32** |  | [optional] [default to null]
**MaxAuditDirSize** | **string** | Maximum audit directory size | [optional] [default to null]
**MaxRetentionPeriod** | **int32** | Max retention period for audit files | [optional] [default to null]
**MaxRetentionTimeunit** | **string** | Max retention period timeunit for audit files | [optional] [default to null]
**ProtocolsAudit** | [***interface{}**](interface{}.md) | Map of protocols audit configurations | [optional] [default to null]
**Protocols** | **[]string** | Protocols to audit | [optional] [default to null]
**DefaultOthersShareLevelPerm** | **string** | Default Share level permissions for Others. (deprecated since 4.6) | [optional] [default to null]
**IsLargeSubnet** | **bool** | Cluster uses a large subnet | [optional] [default to null]
**EkmAddress** | **string** | EKM address | [optional] [default to null]
**EkmPort** | **int32** | EKM port | [optional] [default to null]
**SecondaryEkmAddress** | **string** | Secondary EKM address | [optional] [default to null]
**SecondaryEkmPort** | **int32** | Secondary EKM port | [optional] [default to null]
**EkmServers** | **string** | List of additional EKM servers: 10.0.0.1:5696,11.0.0.1:5697 | [optional] [default to null]
**EncryptionType** | **string** | Encryption type | [optional] [default to null]

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)

